...
// The data is pageData, not a map, because production renders pageData and
// the difference is what made invalid.html's negative assertions vacuous: a
// map missing a key renders as "<no value>", so the forbidden-string checks
...
func TestRenderedPagesAreDistinct(t *testing.T) {
	...
	// filled is what the handler would have on hand at the moment it chooses
	// a page: a resolved library, with a name, an address and a shelf link.
	filled := func(title string) pageData {
		return pageData{
			Title:       title,
	...
	render := func(name string, data pageData) string {
		t.Helper()
	...
	t.Run("outdated.html renders the card-expired copy and the library name", func(t *testing.T) {
		data := filled("Card out of date")
		data.CardLabel = "August 2026"
		data.StoppedOn = "3 September"
		body := render("outdated.html", data)
		for _, want := range []string{
			"This card is out of date.", "Riverside Little Free Library",
			"August 2026", "stopped working on 3 September", "/b/riverside/",
		} {
			if !strings.Contains(body, want) {
	...
	t.Run("notyet.html renders the not-in-use-yet copy", func(t *testing.T) {
		data := filled("Card not in use yet")
		data.CardLabel = "July 2027"
		data.StartsOn = "24 June 2027"
		body := render("notyet.html", data)
		for _, want := range []string{
			"This card isn't in use yet.", "Riverside Little Free Library",
			"July 2027", "starts working on 24 June 2027", "/b/riverside/",
		} {
			if !strings.Contains(body, want) {
				t.Errorf("notyet.html missing %q\n%s", want, body)
			}
		}
		for _, forbidden := range []string{"out of date", "stopped working"} {
			if strings.Contains(body, forbidden) {
				t.Errorf("notyet.html describes a card that has not started as %q\n%s", forbidden, body)
			}
		}
	})

	t.Run("shelf.html renders the shelf's empty state", func(t *testing.T) {
		body := render("shelf.html", filled("Riverside Little Free Library"))
	...
	t.Run("about.html renders the about copy, not the shelf's", func(t *testing.T) {
		body := render("about.html", filled("About Riverside Little Free Library"))
