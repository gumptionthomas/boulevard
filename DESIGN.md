...
**The two printed artifacts have names, not descriptions:** the browse code above is user-facing copy's **shelf code**, and the leave/take code is the **take/leave card**. User-facing copy says one of those, never "the box," "the box door," "the hinge," "in the door," or "Little Free Library." An LFL is one demographic; a Boulevard may be a sandwich board, a garage door, or a fence, and copy that assumes a box excludes every steward who has none. `TestNoTemplateAssumesALittleFreeLibrary` (`internal/web`) and `TestPrintedCopyDoesNotAssumeALittleFreeLibrary` (`internal/booklet`) walk the embedded templates and every printed string for exactly those phrases, from identical ban lists.

**"In the door" is on that list because banning the noun is not enough.** The first sweep removed "the box" and left the metaphor standing in thirteen places, including a button reading *"Put this card in the door"* — which names, for the steward this vocabulary exists to include, a thing they have not got. **The active card is "at the shelf,"** matching the phrase already used for the physical spot; **"in place"** is the alternative where "at the shelf" would be the third *shelf* in one sentence. Both guards ban the phrase, but neither reads Go source — they walk the embedded templates and the printed strings, and eight of those thirteen were Go string literals, `internal/web/steward_items.go`'s `okMessages` flash map among them. The largest cluster of the metaphor sits in the one place no guard is looking.
...
**Control placement: a destructive control at the end of a page never sits within a tap-target of a benign one, and comes last.** 44px, which is more space than anything else on the page puts between two controls, so it reads as a different kind of thing rather than as the next item in a list. The tokens page is why: "Start a new booklet…" used to sit 12px under "Download the booklet," making the two most different actions on the page the two closest together.

**The rule is about a page's terminal controls, not about controls repeated inside a row** — and that exemption is written down here because the alternative is a future reader finding Pin and Remove 12px apart on the steward's shelf, or "Give it another month" and `Revoke…` on the tokens page, and reading it as the rule having been forgotten. Two things make a row different. A repeated control cannot buy separation with space: 44px inside every row multiplies by the row count on the one surface a steward scrolls, and pushing the rows apart is not the same as pushing a mis-tap away. And neither per-row control is what the rule guards against. **Remove is recoverable** — it sheds the item, and re-shelving is one tap from the shed, which is exactly why removal sheds rather than releases (below). **`Revoke…` is irreversible but gated**, by a confirmation page of its own; that page's friction is the protection, and it is deliberately more friction than the terminal asks for. A per-row control that is neither recoverable nor confirmed does not get the exemption — it gets a confirmation page.
...
It also carried a **deliberate look-and-feel sweep**, mostly non-functional, called for after the Milestone 4b acceptance run: the bones were right, but **copy, ordering and type sizes** each needed a pass. Named there or found since — a rotate confirmation whose most consequential sentence was the smallest type on the page; a destructive control sitting closer to a benign one than anything else on the tokens page; copy that assumed a Little Free Library; and the CLI and the desk labelling the same card differently — `cmd/boulevard/tokens.go`'s `cardLabel` prints `Booklet 2 · Card 1`, while `internal/web/templates/steward-tokens.html` prints `Booklet 2 · Card 1 of 12` for the identical card, a bare "of 12" apart. (This is a different bug from the handle mismatch Milestone 4b fixed, where `revoke`'s argument disagreed with the number printed on the card — that one was closed by printing the handle in brackets; this labelling gap is still open.) Delivered: a `.consequence` type class and an explicit size scale; a control-placement rule keeping destructive actions last and 44px clear of anything benign; the vocabulary sweep of §4 across templates and the booklet cover; and an unconditional orientation line, which is also what corrected §6's empty-state claim (above). This was a sweep, not a rewrite — nothing in it changed what the software does. Two items were left for later because they are behavior, not look-and-feel: `boulevard booklet`'s reprint still asking for a base URL that is printed on a mounted sign, and the tokens page accumulating revoked rows after rotation. The CLI/desk label mismatch just named is copy, squarely in this sweep's scope, and simply did not get fixed — record it as still open rather than as done, since it is the kind of thing a milestone note is supposed to catch someone forgetting.
