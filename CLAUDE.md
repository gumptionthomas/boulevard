...
- **A card is named by `printed_from`, never by `valid_from` or `valid_until`.** Both of those move: force-activate rewrites `valid_from`, extend rewrites `valid_until`. Deriving the month label from either renames the card in the steward's hand — the acceptance run found an October card showing as "August 2026" across the tokens page, the booklet PDF, the scan diagnostics and the rotate confirmation, the last of which names the surviving card while asking whether to discard secrets. `printed_from` is set once at mint and never modified; `boulevard.Token.Label()` is the one place it is formatted. DESIGN.md §4's acceptance of the date divergence depends entirely on this, so do not "simplify" a label back onto a date field.
- **A new column on an exported table must be added to `exportedTables` too.** `internal/store/export.go` carries its own column lists, and a column missing from one is dropped from every export. `TestExportCarriesEveryColumnOfEveryExportedTable` reads the live schema through `pragma_table_info` and fails if a list drifts — it exists because `printed_from` drifted within an hour of the review that named this as the top risk.
...
- **Rotate leaves the active card alone.** It deletes every `pending` token and mints twelve fresh ones, but the card currently in the door is untouched by the same transaction. The alternative — replacing all twelve — means an action taken at a keyboard makes the box stop working until someone physically visits it, and the steward who just ran the command may be nowhere near it. Killing the live card stays available, but only as its own deliberate act: revoke.
...
- **The steward key is generated, never chosen, and only its SHA-256 hash is stored.** This is not the same decision as the plaintext token secrets just above, and the two are not in tension: a token secret must stay recoverable because reprinting a lost booklet must always work, while a steward key has no artifact to reprint — losing it just means running `boulevard steward-key` again. There is nothing worth keeping recoverable, so it is not kept. Every steward route 404s, not 403s, until a key exists — a fresh install should not advertise a surface that is not armed. That rule survives a **method mismatch** only because `hideUnmatchedMethods` rewrites `net/http`'s 405 into a byte-identical 404 and drops the `Allow` header. The mux matches methods before any handler runs, so a GET at a POST-only steward mutation never reaches `requireSteward`, and a 405 confirms the route is real — for a slug that need not exist, on a box with no key. Do not remove that middleware as redundant; the handler-level check cannot see these requests.
...
- **No store method infers a current library.** Resolution boundaries (`LibraryBySlug`, `LibraryByID`, `TokenBySecret`, `SessionByID`, `DeleteSession`, `StewardSessionByID`, `DeleteStewardSession`) are handed an identifier and return what it names, so they take no `LibraryID`. Host-scoped queries (`LibrarySlugs`, `SweepExpiredStewardSessions`) take none because the question is about the host. Everything else takes an explicit `boulevard.LibraryID`. Nothing anywhere falls back to "the only library".
